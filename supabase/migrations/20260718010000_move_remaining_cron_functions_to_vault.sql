-- A "rotacao" de 2026-07-13 (rotate_cron_secret_and_lock_rpcs) recolocou o
-- MESMO segredo ja vazado no git em mais 5 funcoes, em vez de gerar um
-- valor novo. Estas 5 funcoes passam a ler o segredo do Supabase Vault
-- (mesma entrada 'cgm_cron_secret' usada por dispatch_cgm_sync desde a
-- migration cgm_sync_dispatcher_vault_secret) em vez de um literal.
...
begin
  select decrypted_secret into cron_secret from vault.decrypted_secrets where name = 'cgm_cron_secret' limit 1;
  if cron_secret is null or p_secret <> cron_secret then
